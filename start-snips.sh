...
ENABLE_HOTWORD_SERVICE=${ENABLE_HOTWORD_SERVICE:-no}
INSTALL_SKILLS=${INSTALL_SKILLS:-yes}

# install all skills
if [ $INSTALL_SKILLS == yes ]; then

	# deploy apps (skills). See: https://docs.snips.ai/articles/console/actions/deploying-your-skills#deploy-manually-without-sam
	...
	# skill deployment is done, go back to root directory
	cd /
fi
